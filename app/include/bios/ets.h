...
int ets_printf(const char *format, ...); // Print formatted data to putc1_routine() + putc2_routine() + putc1_external()
int ets_uart_printf(const char *format, ...); // Print formatted data to uart (_putc1())
void ets_install_external_printf(void *putc1_routine, void *putc2_routine, void *putc2_external); // { putc1_external=r3; if(r3!=NULL) ets_install_putc2(r3); else ets_install_putc2(_putc2_def); putc2_external=a4; }
int ets_external_printf(const char *format, ...); // Print formatted data to eprintf_buf (_putc2_def())
extern char * printf_pbuf; // use _putc2_def
extern char * printf_buf_len; // use _putc2_def
#define external_printf_init_buf(buf, len) do { printf_pbuf = buf; printf_buf_len = len } while(0)
void _putc2_def(char c); // if (printf_buf_len !=0 ) { *printf_pbuf++ = с; printf_buf_len--; }
